const idstatus = (typeof statusblanko === 'undefined') ? '' : statusblanko;
...
function get_and_fill(offset = 0, limit = 10, asuransi = null, nomor = null) {
    ...
    $.get(base_url('api/blanko/' + idstatus + encodeURI(query_string)), function (data, status) {
...
function table_row(id = '', asuransi = '', prefix = '', nomor = '') {
    ...
    table_row += '<span class="text-secondary">' + ((prefix === null) ? '' : prefix) + '</span><b>' + nomor + '</b></td>';
...
function set_action_button(vals, sts = '1') {
    if ((parseInt(sts) > 1)) {
        return '<a href="' + base_url('blanko/detail/' + vals) + '" class="btn btn-info btn-sm text-bold">Lihat Detail<i class="fas fa-info-circle ml-2"></i></a>';
    } else {
