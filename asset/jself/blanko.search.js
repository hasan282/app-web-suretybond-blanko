...
function blanko_action(page = 1) {
    ...
    $('#loading').fadeIn(function () {
        setTimeout(() => {
            get_and_fill(status_val, offset, limit, (asuransi_val == '') ? null : asuransi_val, (number_val == '') ? null : number_val);
        }, 100);
    });
}

function get_and_fill(id_status = 1, offset = 0, limit = 10, asuransi = null, nomor = null) {
    ...
    $.get(base_url('api/blanko/' + id_status + encodeURI(query_string)), function (data, status) {
        if (data.status && status == 'success' && data.data.list.length > 0) {
            let table_rows = '';
            data.data.list.forEach(val => {
                table_rows += table_row(val.id, val.asuransi, val.prefix, val.nomor, [val.status, val.color]);
            });
...
function table_body(rows = '') {
    ...
    table += '<th class="text-center">Status Pemakaian</th><th class="text-center"><i class="fas fa-cog"></i></th>';
    table += '</tr></thead><tbody>' + rows + '</tbody></table>';
    return table;
}

function table_row(id = '', asuransi = '', prefix = '', nomor = '', status = ['', 'dark']) {
    let table_row = '<tr><td>' + asuransi + '</td>';
    table_row += '<td class="text-center"><span class="text-gray">' + prefix + '</span><span class="text-bold">' + nomor + '</span></td>';
    table_row += '<td class="text-center text-bold text-' + status[1] + '">' + status[0] + '</td><td class="text-center py-0 align-middle">';
    table_row += '<a href="' + base_url('process/crash/' + id) + '" class="btn btn-danger btn-sm text-bold">';
    table_row += 'Lapor Rusak<i class="fas fa-exclamation-triangle ml-2"></i></a>';
    table_row += '<a href="' + base_url('blanko/detail/' + id) + '" class="btn btn-primary btn-sm text-bold ml-2">';
    table_row += 'Detail<i class="fas fa-eye ml-2"></i></a>';
    table_row += '</td></tr>';
    return table_row;
}
